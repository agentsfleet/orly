...
| 4.25 (Rust) | Do `*.rs` edits route to `dispatch/write_rust.md` — ownership, error variants, feature combinations, deterministic contention tests — with Microsoft's Pragmatic Rust Guidelines (`~/Projects/oss/rust-guidelines/all.txt`, fetched when absent) read sectioned and its `M-…` IDs cited in every Rust review? | YES | <!-- oracle-packs:language.rust -->
| 4.25b (Rust errors) | Is RULE ERR-RS deterministic where a machine can decide it — `dispatch/write_rust.sh` running `audits/rust-error.sh` over staged `*.rs` to block a `map_err` that stringifies its own cause and an error type with no `pub type Result<…>` in its crate — while `source()` shape and whether a context addition is warranted stay the agent's call? | YES | <!-- oracle-packs:language.rust -->
...
| 10.3 | When a harness/gate/hook fires (`msid-ui.sh`, gitleaks, ZIG GATE, FLL GATE, pre-commit/pre-push, etc.), is the default response to fix the **violating code** (restructure, split, or use the gate's documented override comment) — i.e. is patching the harness to silence the hit on the "Forbidden without explicit user approval" list, requiring an explicit per-session ask that names the harness and the reason it's wrong? | YES |
...
| 23.9 | Is live-eval token spend controlled by keeping it off the push path entirely (rationale recorded in `.githooks/pre-push`), with `make llmevals SMOKE=1` and the full `make llmevals` matrix available as explicit manual runs? | YES |
...
Added by M02_001. Two failures motivated it: nobody could answer "is
LOGGING_STANDARD adhered to, and which clauses?" without reading audit source
by hand, and the `📖 DOC READ` proof-line was a claim an agent made about
itself, compared against nothing. The ledger answers the first in a committed
file; the read record answers the second mechanically, in every runtime rather
than only the one with a Read hook.
