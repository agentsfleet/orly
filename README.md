...
[![npm](https://img.shields.io/npm/v/@agentsfleet/orly?logo=npm&logoColor=white&label=%40agentsfleet%2Forly)](https://www.npmjs.com/package/@agentsfleet/orly)
...
That is the whole install. It writes the rule pages, the gate scripts, the
skills those rules name, and git hooks — for your languages, read from your
own sources. A Rust crate receives the Rust rules and never the Zig ones. No
checkout of this repository and no prepared `$HOME`: the same command works on
a fresh machine, a Continuous Integration (CI) runner, or a remote container.

**Everything it writes is meant to be committed.** That is how the rules reach
your teammates — they clone, and the rules are already there. The one thing a
clone cannot carry is `core.hooksPath`, which is local git config, so each
person runs `orly init` once to arm the hooks.
...
An existing `AGENTS.md` keeps its name and its bytes and gains the pointer. If
you had none, you get a stub with room to write your own rules later. Yours
...
Which packs you get is read from your own sources. Opt-in packs never
auto-select; name one with `--with`.
...
number, the workflow enforces it. A merge to `main` carrying a new
...
`--no-hooks` because it wrote its own `.githooks/`, and orly refuses to replace
hooks it did not write:
