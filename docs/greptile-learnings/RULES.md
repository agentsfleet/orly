...
**Tags:** zig, js, all
**Example:** TS/JS — unused deps in agent.js; dead currentObj branch in simpleYamlParse. dead CLI variables.
...
**Example:** User directive (Indy, Jun 04, 2026) — a well-named identifier beats a comment; a comment earns its place only by preventing future-agent confusion.
...
**Example:** reload_pending: ?*T = null with one always-non-null caller (worker_watcher.spawnZombieThread) — original commit followed cancel_flag's optional pattern, user called it dead defense. Same struct's cancel_flag/worker_state/executor follow the same anti-pattern; NLR mandates same-diff cleanup.
...
**Example:** TS/JS — extractCreatedAt used indexOf on JSON. simpleYamlParse silently dropped all arrays.
...
**Tags:** zig, memory
**Example:** Zig — manual alloc.free() + errdefer on same pointer = double-free. bvisor Thread.zig, ThreadGroup.zig for ref/unref pattern.
<!-- oracle-packs:end -->
...
**Tags:** zig, security
**Example:** Zig — webhook Bearer token compared with std.mem.eql.
...
**Example:** readMessage returned null for ConnectionResetByPeer → busy-loop. streamRunWatch treated 503 = 404.
...
**Tags:** zig, sql
**Example:** unqualified platform_llm_keys in handler query.
...
**Tags:** sql, zig
**Example:** activity_stream cursor was timestamp-only, dropped events at ms boundaries.
...
**Example:** Zig — flagged on after existing violations `event_loop_m23_integration_test.zig` and `m24_001_cross_workspace_idor_test.zig` surfaced. User directive while writing `test "3.3: Jira valid HMAC → .next"` — renamed in the same branch.
...
**Example:** heartbeat 30s > socket timeout 25s → proxy dropped connection.
...
**Tags:** zig, js, testing
**Example:** Zig CLI buffered entire SSE response, printed all events at once.
...
**Tags:** js
**Example:** abortedRef boolean was frozen at false inside called function.
...
**Example:** resolveSlot read partially-written fields after losing CAS.
...
**Tags:** js, cli
**Example:** undocumented AGENT_ERROR/IO_ERROR codes; usage text returned as error message.
...
**Tags:** sql
**Example:** Zig — original bug. `sql_splitter.zig` + unit test in `common.zig` replaced this rule.
...
**Tags:** bash, ci
**Example:** PR #162 glob matched itself → fork bomb in CI.
...
**Refs:**
- Zig — handleReceiveWebhook had 7 inline strings including "Bearer ", status values, and error codes.
- a progress-callbacks test re-stringified `event_received` / `tool_call_started` / `chunk` / `tool_call_completed` / `event_complete` instead of importing `activity_publisher.KIND_*`. Fix: promoted the consts to `pub`, imported them. Caught only at human review — pre-edit grep would have caught it automatically.
...
**Tags:** zig
**Example:** webhook handler mixed ERR_* constants with inline message strings.
...
**Example:** Principle — no single incident yet.
...
**Tags:** zig
**Example:** ZombieTrigger struct with ?source/?schedule — webhook-without-source was valid but semantically wrong.
...
**Example:** webhook_secret TEXT column in core.agents → refactored to webhook_secret_ref.
...
**Tags:** sql
**Example:** DEFAULT 'active' and CHECK status IN (...) removed from core.agents.
`current_setting('fleet.allow_gate_purge')` in schema/810 + schema/830
...
**Tags:** zig, security
**Example:** writeJsonString only escaped " and \ — \n in YAML value could inject JSON keys.
...
**Tags:** zig, security
**Example:** constantTimeEq skipped XOR loop entirely on length mismatch.
...
**Tags:** zig, safety
**Example:** Zig — `WebhookSig` used `*anyopaque` for lookup context; refactored to `pub fn WebhookSig(comptime LookupCtx: type) type` — the host passes `*pg.Pool` directly with no cast.
...
**Tags:** zig
**Example:** ZombieRow used []u8 for workspace_id, status, token — all immutable DB data.
...
**Tags:** zig, js, sql, all
**Example:** webhook_secret renamed but stale comments and test fixtures still used the old name.
...
**Tags:** process
**Example:** multiple follow-up fix commits after missing orphan sweep in CHORE(close).
...
**Tags:** zig, testing
**Example:** router tests existed since but never ran; two pre-existing bugs surfaced on import.
...
**Status:** Eliminated. Use `src/db/pg_query.zig` — helpers take `*PgQuery`, not `anytype`. `q.next()` always works; the `q.*.next()` footgun is structurally impossible.
**Why the old rule existed:** q.next() on a pointer type compiles but calls the wrong dispatch.
**Tags:** zig, postgres
**Example:** Zig — original bug. PgQuery wrapper eliminates this class.
...
**Tags:** zig
**Example:** Zig 0.15 breaking API change from 0.13.
...
**Tags:** zig
**Example:** AnomalyRule.behavior was always .auto_kill at every site — field removed.
...
**Tags:** zig, js
**Example:** AnomalyRule.pattern was []const u8 with only one valid value; ApprovalPayload.decision validated late.
...
**Tags:** zig
**Example:** Zig — slack_verify.zig Provider enum rewrote as webhook_verify.zig VerifyConfig struct.
...
**Tags:** zig, js, testing
**Example:** agentmail domain was .to in prod, .dev in tests, .com in spec docs.

## RULE ERH — ~~Every ERR_* code must have a hint() entry~~ ELIMINATED

**Status:** ELIMINATED — Entry struct requires `.hint` field; comptime asserts `hint.len > 0`.
**Tags:** zig
...
**Tags:** zig, security
**Example:** Zig — 64KB normalization buffer in injection_detector.zig bypassed via 65KB padding.
...
**Tags:** zig
**Example:** globMatch silently treated mid-path wildcards as exact match — fixed in parseEndpointRules.
...
1. **Result variants** returned from a function that mean "we took a non-default path" — `.truncated`, `.fallback`, `.degraded`, `.partial`, `.skipped`, `.deduped`, `.ignored`, `.rate_limited`, `.retry_scheduled`. The originating incident.
...
## RULE SNT — ~~Registry sentinels must be distinct from real entries~~ ELIMINATED

**Status:** ELIMINATED — UNKNOWN sentinel is defined outside REGISTRY; comptime assertion prevents collision.
...
**Example:** Zig — error_table.zig — lookup() replaced O(n) for-loop with StaticStringMap(usize) mapping code→TABLE index.
...
**Tags:** zig, api-design
**Example:** Zig — hx.zig — db()/releaseDb() shipped with zero callers, all handlers used ctx.pool directly. Removed in greptile fix.
...
**Example:** check_openapi_errors.py — $ref responses were silently skipped; added target validation.
...
**Tags:** zig, testing
**Example:** Zig — error_registry_test.zig — renamed "does not start with UZ- — wait, it does" to "has sentinel code UZ-UNKNOWN and is 500".
...
**Example:** Zig — workspace_credentials_http.zig — second DELETE swallowed the error; ROLLBACK was missing. Caught by greptile on PR #189.
...
**Example:** Zig — bvisor src/core/Supervisor.zig, Thread.zig, ThreadGroup.zig, OverlayRoot.zig, LogBuffer.zig.
...
**Example:** Zig — integration_grants.zig:handleRequestGrant. Fixed PR #205.
...
**Example:** Zig — grant_approval_webhook.zig:verifyAndConsumeNonce. Fixed PR #205 (initial GETDEL, second greptile P1 review 4095571471).
...
**Example:** Zig — webhook_verify.zig — `isTimestampFresh` accepted future timestamps. Fixed in d07b6de.
...
**Example:** Zig — slack_oauth.zig `extractWorkspaceId` — `catch ""` on dupe failure. Fixed in d07b6de.
...
**Example:** schema/028_workspace_integrations.sql — missing GRANT for api_runtime and worker_runtime. Fixed in this PR.
...
**Example:** Zig — slack_oauth_client.zig `exchangeCode` — `code` interpolated raw. Fixed in this PR.
...
**Example:** Zig — `webhook_url_secret.zig::constantTimeEq` — comment corrected to note that length is still leaked via loop iteration count.
...
**Example:** Zig — — `stage_id`/`role_id`/`skill_id` removed from StartStage; stale `object.put("session_id", ...)` puts in `integration_test.zig`, `handler_edge_test.zig`, `handler_negative_test.zig`, `crash_test.zig` caught by greptile post-merge.
...
**Why:** The handler-shape sweep moved every handler from `handleXxx(ctx, req, res, ...)` to `innerXxx(hx, req, ...)`. Re-introducing raw `common.writeJson`/`common.errorResponse` drags back the old signature and bypasses the JSON envelope / RFC 7807 contract that `Hx` enforces. Greptile will flag reviews that do this.
...
**Example:** –5.2 — full sweep of 18 handler files to inner*/hx. Canonical guide: `docs/REST_API_DESIGN_GUIDELINES.md` §8.
...
**Why:** The steer endpoint shipped with `ack: true` (redundant) and `run_steered` (ambiguous dual-semantics). Caught only in post-build audit against docs/REST_API_DESIGN_GUIDELINES.md. A pre-write checklist would have caught both at design time.
...
**Example:** Zig — `agent_steer_http.zig` — `ack` dropped, `run_steered` split into `message_queued` + `execution_active` after post-build audit. `docs/REST_API_DESIGN_GUIDELINES.md`.
...
**Don't:** Copy-paste a pre-sweep handler (anything that takes `ctx *Context` as first param, or calls `common.writeJson` directly). Those are the old pattern.
**Tags:** zig, http, handlers, hx, api-style
**Example:** `docs/REST_API_DESIGN_GUIDELINES.md` §7–§8. RULE HXX (same topic, handler signature). full sweep.
...
**Example:** Zig — pR #217 greptile comment `3089018810` — `agent_activity_api.zig:innerListActivity` missing check. Complements RULE WAUTH.
...
**Example:** Zig — pR #217 greptile comment `3089018915` — `agent_activity_api.zig` ↔ `activity_stream.zig:queryByAgent`. Fixed by adding `queryByAgentOnConn`.
...
**Example:** Zig — pR #221 greptile comment `3094814139` — `agent_billing_summary.zig` missing operator guard. Fixed by switching to `workspace_guards.enforce(.minimum_role = .operator)`.
...
**Example:** Zig — pR #221 greptile comment `3094604729` — `agent_billing_summary.zig::parsePeriodDays` accepted `{7, 30}` while `workspaces_billing_summary.zig::parsePeriodDays` accepted `{7, 30, 90}`. Fixed by adding `90` to the agent handler and the OpenAPI enum.
...
**Example:** PR #221 greptile comment `3094604881` — `ConfirmDialog` used `id="confirm-dialog-desc"`. Fixed with `React.useId()`.
...
**Example:** PR #221 greptile comment `3094605030` — `ConfirmDialog::handleConfirm` used `try/finally` without `catch`, contradicting its JSDoc. Fixed with `catch` + `onError` callback.
...
**Why:** Silent test swallow is a class-C outage — the gate claims green while regressions ship. Observed where `@python3 audits/test_check_openapi_sync.py 2>&1 | tail -3` in `make openapi` passed a failing-test injection without aborting. Greptile P1 caught it before merge; the root cause (Make + default `sh` POSIX pipefail semantics) is the same bug anywhere a recipe uses `|` to tidy long output.
**Tags:** make, ci, testing
**Example:** `agentsfleet` P2_INFRA_M28_003 §2 — `make/quality.mk:161` before fix piped the test runner through `tail -3`; fix in commit 66556e99 dropped the pipe, confirmed with injected `self.fail()` that `make: *** [openapi] Error 1` now fires correctly.
...
**Example:** PR #252. Greptile P1 finding `3143083466` on `feat/m45-vault-structured`. Fix in commit (this commit) — `validateCredentialName` now rejects `name == "llm"`; integration test asserts both the 400 response and the absence of an `agent:llm` row.
...
**Example:** Pull Request (PR) #278 (May 01, 2026). Three review rounds reframed the platform-managed Large Language Model (LLM) api_key as a magic constant, "loaded at API boot from server config," and "platform vault at platform-scope identifier" before the user pointed at `playbooks/operations/admin_bootstrap/001_playbook.md` + `schema/006_platform_llm_keys.sql` + `docs/v2/done/_P1_API_AUTH_BIL_BOOTSTRAP_REMOVAL_AND_BALANCE_GATE.md`. Each prior framing was internally consistent within its own doc but contradicted the locked decision (admin user signs up like any user, stores credential in own workspace vault, registers via `PUT /v1/admin/platform-keys`, the `platform_llm_keys` table stores only a pointer). The pattern — skip the playbook + done-spec walk, invent a fresh framing — is fully generalisable across any cross-cutting domain, so the rule is intentionally domain-agnostic.
...
**Example:** TS/JS — pR #519 (Jul 14, 2026), greptile P2 (re-ranked to correctness). `countFleets` in `ui/packages/app/lib/fleet-rollup.ts` — the function added *specifically* to make the dashboard rollup total over `AGENTSFLEET_STATUS` — tested `fleet.status in byStatus`. A fleet whose status was `constructor` counted as known, so the totals stopped reconciling in the one place built to prove they always would. Fix: `Object.hasOwn`, pinned by an `it.each` over four `Object.prototype` members that fails 4/4 on the old check.
