...
Anonymous events enter a bounded local queue under the agentsfleet state root.
The sender receipts the exact batch before it posts to PostHog's public capture
endpoint. A successful response removes the acknowledged prefix. Queue
maintenance drops records older than seven days and keeps the file at or below
10 MiB, so an unavailable destination cannot create an unbounded local log.
